...
from utils.keypoint_augmentation import ScaleAug, RotateAug, CropAug, FlipAug,joints_to_point8, point8_to_joints, AugImgMetadata
from utils.keypoint_heatmap import create_heatmap
from dataflow.keypoint_dataflow  import CocoDataFlow, JointsLoader, COCODataPaths

from utils.preprocessing_image import preprocess_image

ALL_HEATMAP_MASK = np.repeat(
    np.ones((120, 120, 1), dtype=np.float32), 19, axis=2)
...
def read_img(components):
    ...
    img_buf = cv2.imread(meta.img_path)
    if img_buf is None:
