...
from utils.retinanet import losses
from utils.retinanet import models
from utils.retinanet.callbacks import RedirectModel
from utils.retinanet.callbacks.eval import Evaluate
from network.retinanet import retinanet_bbox
from utils.retinanet.utils.anchors import make_shapes_callback
from utils.retinanet.utils.config import read_config_file, parse_anchor_parameters
from utils.retinanet.utils.model import freeze as freeze_model
from utils.retinanet.utils.transform import random_transform_generator
...
def create_callbacks(model, training_model, prediction_model, validation_generator, args):
    ...
    if args.evaluation and validation_generator:
        if args.dataset_type == 'coco':
            from utils.retinanet.callbacks.coco import CocoEval
    ...
    if args.snapshots:
        ...
        checkpoint = keras.callbacks.ModelCheckpoint(
            os.path.join(
                args.snapshot_path,
                '{backbone}_{dataset_type}_{{epoch:02d}}_{{val_loss:.2f}}.h5'.format(backbone=args.backbone, dataset_type=args.dataset_type)
            ),
            verbose=1,
            # save_best_only=True,
            # monitor="mAP",
...
def create_generators(args, preprocess_image):
    ...
    if args.dataset_type == 'coco':
        ...
        from utils.retinanet.preprocessing.coco import CocoGenerator
