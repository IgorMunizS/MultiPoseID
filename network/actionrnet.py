...
class ActionRNet():
    ...
    def __init__(self, nb_actions, nb_keypoints, bck_arch = 'resnet50', prediction = False, bck_weights=None, keypoint_weights=None):
        self.nb_keypoints = nb_keypoints + 1
        if prediction:
        ...
        #backbone.load_weights(bck_weights, by_name=True)
        ...
        self.keypoint_model = Model(inputs=[input_graph], outputs=[self.D])
        if keypoint_weights:
            self.keypoint_model.load_weights(keypoint_weights,by_name=True)

        pose_features = self.keypoint_model.output
        backbone_features = C5
        glave_pose = KL.Flatten()(pose_features)
        glave_back = KL.Flatten()(backbone_features)
        x = KL.concatenate([glave_pose, glave_back])
        x = KL.Dense(1024, activation='relu')(x)
        x = KL.Dropout(0.3)(x)
        x = KL.Dense(nb_actions, activation='softmax')(x)


        self.model = Model(inputs=[input_graph], output = [x,glave_pose,self.D, glave_back])
